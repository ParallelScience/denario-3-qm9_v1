...
To isolate the non-additive electronic contributions to the internal energy (`u0`), the extensive scaling inherent to the QM9 dataset was decoupled using an additive baseline model based on group-contribution counts. The performance metrics of this Ridge regression baseline, alongside the non-linear Random Forest baseline and the Graph Neural Network (GNN), are recorded in `data/ridge_metrics.txt`, `data/rf_baseline_metrics.txt`, and `data/gnn_metrics.txt`, respectively, though they were not printed to the standard output. By standardizing the residuals of the linear baseline, the subsequent GNN was restricted entirely to learning topological and non-additive electronic physics, rather than basic molecular size scaling.
...
*   **Fluorine**: Molecules with fluorine show a narrower gap (mean 0.2133 Hartree vs. 0.2519 Hartree, ANOVA $p = 0.0000$, Cohen's $d = -0.7677$) and a marginally increased dipole (mean 2.9627 Debye vs. 2.7084 Debye, ANOVA $p = 0.0162$, Cohen's $d = 0.1728$).
